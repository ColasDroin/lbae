...
import pymzml
import pyimzml
import modules.tools.external_lib.ImzMLParser as ImzMLParser
...
class SmzMLobj:
    ...
    def __init__(
        ...
        **pymzml_kwargs: Dict[Any, Any],
        ...
        if annotation.lower().endswith("imzml"):
            self.reader = ImzMLParser.ImzMLParser(
                annotation, ibd_file=self.filepath, parse_lib="lxml"
            )
        ...
        if annotation is None:
            ...
            try:
                ...
                logging.warning(
                    "The annotation dictionary has faulty keys. To check metadata, use attribute annot_dict of the smz object"
                )
    ...
    def _generate_sparse_intensities(self) -> sparse.lil_matrix:
        ...
        for i, spectrum in progressbar(enumerate(self.spectra), total=len(self.spectra_original)):
            ixes, idx, idx_found = spectrum.sparse_ixes(mz_reference=self.mz_vals)

            # iterpolate
            # ix_above0 = np.argwhere(self.spectra_original[i].i > 0).flatten() # identify spectra's signal indexes
            # sequences = [list(group) for group in mit.consecutive_groups(ix_above0)]

            # mz_vals_select = np.zeros_like(self.mz_vals, dtype=bool) # boolean array with True at the indexes were interpolation should occur
            # for s in sequences:
            #     min_, max_ = self.spectra_original[i].mz[s[0]], self.spectra_original[i].mz[s[-1]] # select the boundaries for the peak
            #     ix_ = np.argwhere((self.mz_vals >= min_ )& (self.mz_vals <= max_)).flatten() # identify which indexes in mz_vals this corresponds to
            #     mz_vals_select[ix_] = True

            # function_interp = interp1d(self.spectra_original[i].mz, self.spectra_original[i].i, kind='linear', bounds_error=False, fill_value=0.0)
            # output = function_interp(self.mz_vals)#[mz_vals_select])

            # print(self.spectra_original[i].mz, self.spectra_original[i].i)
            # print('\n\n\n')
            # print(self.mz_vals[ixes[:idx]])
            # print('\n\n\n')
            # print(output)
            # break

            S[i, ixes[:idx]] = spectrum.i[idx_found]
            # S[i, :] = output# spectrum.i[idx_found]
    ...
    def _load(self, ixs2load: np.ndarray, show_progress: bool = False) -> None:
        ...
        if show_progress:
            with progressbar(
                ...
                if type(self.reader) != ImzMLParser.ImzMLParser:
                    for i, spectrum in enumerate(self.reader):
                        if i in set_ixs2load:
                            self.spectra[cnt] = Spectrum(
                                spectrum.mz - self.mz_shift[cnt], spectrum.i
                            ).resolved(self.mz_resolution)
                    ...
                    for i, (x, y, z) in enumerate(self.reader.coordinates):
                        if i in set_ixs2load:
                            ...
                            self.spectra[cnt] = Spectrum(
                                mz - self.mz_shift[cnt], intensity
                            ).resolved(self.mz_resolution)
                            cnt += 1
                            pbar.update(1)
                            if cnt == n_toload:
            ...
            if type(self.reader) != ImzMLParser.ImzMLParser:
                for i, spectrum in enumerate(self.reader):
                    if i in set_ixs2load:
                        self.spectra[i] = Spectrum(
                            spectrum.mz + self.mz_shift[i], spectrum.i
                        ).resolved(self.mz_resolution)
                        cnt += 1
                    if cnt == n_toload:
                        break
                ...
                for i, (x, y, z) in enumerate(self.reader.coordinates):
                    if i in set_ixs2load:
                        mz, intensity = self.reader.getspectrum(i)
                        self.spectra[cnt] = Spectrum(mz - self.mz_shift[cnt], intensity).resolved(
                            self.mz_resolution
                        )
                        cnt += 1
                        if cnt == n_toload:
    ...
    def visualize_bin_range_deviation_intensity(
        ...
        if plot_graph:
            ...
            for i, row in enumerate(S_pixels):
                ...
                if plot_style == "com":
                    ...
                    ax.scatter(
                        intensity, deviation, color=selected_colors[i], marker="x", alpha=alpha
                    )
        ...
        for i in range(len(percentiles_)):
            if i == len(percentiles_) - 1:
                ...
                idx_pixel = np.argwhere(
                    (IMG_SUM > percentiles_[i]) & (IMG_SUM < percentiles_[i + 1])
                ).flatten()
            S_pixels = self.S[idx_pixel][:, INDEX.squeeze()]
