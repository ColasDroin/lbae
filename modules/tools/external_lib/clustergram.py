...
def Clustergram(
    ...
    (fig, ct, curves_dict) = _Clustergram(**kwargs).figure(computed_traces=computed_traces)
...
class _Clustergram:
    ...
    def figure(self, computed_traces=None):
        ...
        if self._row_colors:
            self._row_colors = [
                self._row_colors[r] if r < len(self._row_colors) else "gray" for r in self._row_ids
            ]
        ...
        fig["layout"]["xaxis1"].update(domain=[0, 0.95])  # pylint: disable=invalid-sequence-index
        fig["layout"]["xaxis3"].update(  # pylint: disable=invalid-sequence-index
            domain=[row_ratio + row_colors_ratio, 0.95], anchor="y9"
        )
        fig["layout"]["xaxis5"].update(domain=[0, 0.95])  # pylint: disable=invalid-sequence-index
        ...
        fig["layout"].update(paper_bgcolor=self._paper_bg_color, plot_bgcolor=self._plot_bg_color)
    ...
    def _get_row_colors_heatmap(self):
        ...
        return go.Heatmap(z=z, colorscale=colorscale, colorbar={"xpad": 100}, showscale=False)
    ...
    def _compute_clustered_data(self):
        ...
        if self._cluster in ["col", "all"]:
            cols_dendro = ff._dendrogram._Dendrogram(
                ...
                linkagefun=lambda d: self._link_fun(d, optimal_ordering=self._optimal_leaf_order),
